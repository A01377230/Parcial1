let BotonMinus = document.getElementById("btn_mayusculas");
BotonMinus.addEventListener("click", convertMinus);
function convertMinus(param){
  param.preventDefault()
  let texto = document.getElementById("txt_may").value
        const separar = texto.split(",")
        for (i=0;i<separar.length; i++){
          textoFin= separar[i].toLowerCase()
          separar[i]=textoFin
        }
        document.getElementById("txt_may").value = separar.toString();
       }
      /*
        ...
        */
        ...
        let botonPerro = document.getElementById("btn_añoshumano");
        botonPerro.addEventListener("click",calculateDogAge)
        function calculateDogAge(param){
           param.preventDefault()
           const añosHumanos= document.getElementById("edad_humano").value;
           const humanoInt = parseInt(añosHumanos)
           const añosPerro = humanoInt*7
